            ...
            <form action="{{route('contact.store')}}" method="post" class="bg-white p-5 contact-form">
              ...
              <div class="form-group">
                @error('email')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
                <input type="text" class="form-control" placeholder="Your Email" name="email" value="{{Auth::user()->email}}">
              </div>
              <div class="form-group">
                @error('subject')
                    <div class="alert alert-danger">{{ $message }}</div>
              ...
              <div class="form-group">
                @error('content')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
                <textarea id="" cols="30" rows="7" class="form-control" placeholder="Message" name="content">{{old('content')}}</textarea>
              </div>
              <div class="form-group">
